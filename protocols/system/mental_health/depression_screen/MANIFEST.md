# Depression Screening Intake — Behavioral Manifest

**Pack ID:** depression_screen
...
Governs the intake and documentation of a depression screening — capturing the symptom profile across PHQ-9 domains, the functional impact, the duration and trajectory, the comorbid conditions, the safety assessment, and the treatment history to produce a depression screening profile with severity classification and clinical considerations for the treating clinician.

Depression screening that produces only a score misses the person behind the number. A PHQ-9 score of 15 tells the clinician the severity is moderately severe. It does not tell them that the patient stopped playing with their children three months ago, that they have been calling in sick to work every Monday, or that the weight gain is from a medication they were prescribed for something else. The intake captures both the score and the story — because the treatment plan needs both.
...
---
*Depression Screening Intake v1.0 — TMOS13, LLC*
*Robert C. Ventura*
