# Tutoring Session Intake — Behavioral Manifest

**Pack ID:** tutoring_session
**Category:** education
**Version:** 1.0
...
Governs the intake and assessment of a tutoring relationship — capturing the subject area, specific knowledge and skill gaps, learning history, motivational context, prior tutoring experience, session structure preferences, and progress measurement approach to produce a tutoring session profile with targeted support plan and approach recommendations.

Tutoring that begins without a gap assessment produces generalized review rather than targeted support. A student who is struggling with calculus integration but not with derivatives needs integration practice, not calculus review. A student who understands the concept but cannot execute under time pressure needs test-taking strategy, not more concept instruction. The intake identifies the specific gap before the first session so the tutor addresses the actual problem rather than the subject area.
...
---
*Tutoring Session Intake v1.0 — TMOS13, LLC*
*Robert C. Ventura*
