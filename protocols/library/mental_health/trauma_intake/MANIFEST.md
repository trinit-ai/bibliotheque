# Trauma Assessment Intake — Behavioral Manifest

**Pack ID:** trauma_intake
...
Governs the intake and documentation of a trauma assessment — capturing the trauma history, the current PTSD symptom profile across the four DSM-5 clusters (intrusion, avoidance, negative cognitions and mood, and hyperarousal), the functional impairment, safety considerations, and comorbid conditions to produce a trauma assessment profile with symptom summary and clinical considerations for the treating provider.

Trauma assessment requires a different approach than other clinical intakes. The patient may be describing experiences that produced significant harm. The pacing of the assessment, the choice of language, the amount of detail requested, and the clinician's response to disclosure all affect whether the patient feels safe enough to provide accurate information — and whether the assessment itself is retraumatizing or begins the therapeutic relationship. The intake supports the trauma-informed approach throughout.
...
---
*Trauma Assessment Intake v1.0 — TMOS13, LLC*
*Robert C. Ventura*
